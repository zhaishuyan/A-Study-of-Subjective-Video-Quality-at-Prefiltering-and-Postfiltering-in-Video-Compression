...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*', '*')))):
        ...
        cv2.imwrite(f'{save_dir}/{imgname}_SwinIR.bmp', output)
...
def setup(args):
    pipeline = args.folder_lq.split('/')[-1] if args.folder_lq.split('/')[-1] else args.folder_lq.split('/')[-2]
    ...
    # 003 real-world image sr
        ...
        save_dir = f'results/{pipeline}_swinir_{args.task}_x{args.scale}'
        if args.large_model:
            save_dir += '_large'
...
def get_image_pair(args, path):
    ...
    imgname = path.split('/')[-2] + '_' + imgname

    # 001 classical image sr/ 002 lightweight image sr (load lq-gt image pairs)
    if args.task in ['classical_sr', 'lightweight_sr']:
        img_gt = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.
    ...
    # 003 real-world image sr (load lq image only)
        ...
        img_lq = np.array(Image.open(path).convert('RGB'), dtype=np.float32)[:, :, ::-1] / 255.0
